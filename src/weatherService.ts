...
export class OpenMeteoWeatherService {
  ...
  async getWeeklyForecast(startDate: string, days = 7): Promise<Map<string, HourlyForecast[]>> {
    ...
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${this.lat}&longitude=${this.lon}&hourly=temperature_2m,relative_humidity_2m,precipitation_probability,precipitation,cloud_cover&timezone=auto&forecast_days=${days}`;
    ...
    try {
      ...
      console.warn("[OpenMeteoWeatherService] Error fetching weather, falling back to mock:", err);
...
export async function getHourlyForecast(dateIso: string, lat?: number, lon?: number): Promise<HourlyForecast[]> {
  ...
  const map = await service.getWeeklyForecast(dateIso, 1);
  return map.get(dateIso) || new MockWeatherService("sunny").getHourlyForecast(dateIso);
}
